...
Delegate the ranking via Task tool with `subagent_type="pipeline-analyst"` (bundled with this plugin as of 2026-09-15 — no longer a companion-detection case). Pass the user-context path, 90d time window, focus filter "active deals + tier-A overdue + ICP-fit candidates," and `top-n=10` (we only show 3, but get a deeper list to filter).
